...
testing("min", function()
   local int64 = ffi.new("int64_t[2]", 1234, 5678)
   assert.equals(int64[0], 1234)
   assert.equals(int64[1], 5678)
   assert.throws("bad argument #1 to 'min' \\(number expected, got cdata\\)", function()
      math.min(int64[0], int64[1])
   end)
   assert.equals(util.min(int64[0], int64[1]), 1234)
   assert.equals(util.min(int64[1], int64[0]), 1234)
end)
...
testing("max", function()
   ...
   assert.equals(int64[0], 1234)
   assert.equals(int64[1], 5678)
   assert.throws("bad argument #1 to 'max' \\(number expected, got cdata\\)", function()
      math.max(int64[0], int64[1])
   end)
   assert.equals(util.max(int64[0], int64[1]), 5678)
   assert.equals(util.max(int64[1], int64[0]), 5678)
end)
...
testing("throw", function()
   ...
   -- a stringified error contains the message and the stack traceback
   assert.match("^runtime error\nstack traceback:", tostring(e))
   ...
   local f = make_thrower(util.throw, "this is an error message")
   local ok, e = pcall(f)
   assert.is_false(ok)
   assert(util.is_error(e))
   ...
   assert.match(traceback_pattern, tostring(e))
   -- the traceback is also available separately
   assert.match(traceback_pattern, e.traceback)

   -- rethrowing an error
   e.addon = "raiden"
   local f = make_thrower(util.throw, e)
   local ok, e = pcall(f)
   assert.is_false(ok)
   assert(util.is_error(e))
   assert.equals(e.message, "this is an error message")
   assert.equals(e.addon, "raiden")
...
testing("throwat", function()
   local f = make_thrower(util.throwat, 2, "this is an error message")
   local ok, e = pcall(f)
   assert.is_false(ok)
   assert(util.is_error(e))
   assert.equals(e.message, "this is an error message")
...
testing("pcall", function()
   ...
   local f = make_thrower(util.throwat, 2, "joie")
   local ok, e = util.pcall(f)
   assert.is_false(ok)
   assert(util.is_error(e))
