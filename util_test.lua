...
testing("throw", function()
   ...
   assert.equals(e.message, "my-error")
...
testing("throw1", function()
   local f = make_thrower(util.throw1, { a=5, b=8, class = make_thrower, message = "boo" })
   local ok, e = pcall(f)
   assert.is_false(ok)
   assert(util.is_error(e))
   assert.equals(e.class, make_thrower)
   assert.equals(e.message, "boo")
...
testing("throw1at", function()
   local f = make_thrower(util.throw1at, 2, { a=5, b=8, class = make_thrower, message = "boo" })
   local ok, e = pcall(f)
   assert.is_false(ok)
   assert(util.is_error(e))
   assert.equals(e.class, make_thrower)
